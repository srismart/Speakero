...
def test_watchdog_stops_session_and_emits_auto_stopped(monkeypatch):
    ...
    async def fake_emit(event, data, room=None):
        emitted.append((data.get("type"), data, room))
    ...
    with TestClient(main.fastapi_app) as client:
        ...
        sess.audio_ws = fake_ws
        assert sess.active is True
        time.sleep(0.5)
        assert sess.active is False
    auto_stops = [(t, d, r) for (t, d, r) in emitted if t == "auto_stopped"]
    assert auto_stops and auto_stops[0][2] == "s-quiet"
    assert auto_stops[0][1]["timeout_seconds"] == 0.05
    assert fake_ws.closed is True
    main.SESSIONS.pop("s-quiet", None)


def test_offline_mode_updates_last_final_at(monkeypatch):
    import asyncio
    ...
    sess = main.SessionState()
    sess.start()
    sess.last_final_at = 1.0
    asyncio.run(main._offline_mode(FakeWS(), sess, "sid-x"))
    assert sess.last_final_at > 1.0


def test_env_float_guards_bad_values(monkeypatch):
    monkeypatch.delenv("SPKR_TEST_F", raising=False)
    assert main._env_float("SPKR_TEST_F", 180.0) == 180.0
    monkeypatch.setenv("SPKR_TEST_F", "")
    assert main._env_float("SPKR_TEST_F", 180.0) == 180.0
    monkeypatch.setenv("SPKR_TEST_F", "abc")
    assert main._env_float("SPKR_TEST_F", 180.0) == 180.0
    monkeypatch.setenv("SPKR_TEST_F", "90")
    assert main._env_float("SPKR_TEST_F", 180.0) == 90.0


def test_frontend_handles_auto_stopped_event():
    html = open("static/index.html", encoding="utf-8").read()
    assert "case 'auto_stopped'" in html
    assert "timeout_seconds" in html
